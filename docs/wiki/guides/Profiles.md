...
When rendering PDFs, PageMD uses headless Chrome which may not have access to all system fonts. To guarantee font availability, embed fonts directly using CSS `@font-face` with base64 data URIs.

#### Why Embed Fonts?

CSS `font-family` silently falls back when fonts aren't available:
```css
font-family: 'Inter', 'Segoe UI', sans-serif;
/*           Missing?  Fallback   Generic fallback */
```

There's no error—the browser just uses the next available font. To ensure consistent output, embed the fonts you need.

#### Step 1: Obtain Font Files

Download font files in `.woff2` format (best compression). Most open-source fonts use the [SIL Open Font License](https://scripts.sil.org/OFL) which permits redistribution.

Create a fonts directory in your project:
```bash
mkdir -p .pagemd/fonts
```

Place font files there (e.g., `Inter-Regular.woff2`, `Inter-Bold.woff2`).

#### Step 2: Create Font CSS with Base64 Encoding

Base64 encoding embeds the font data directly in CSS, eliminating path resolution issues.

Create `.pagemd/styles/fonts.css`:

```bash
# Generate base64-encoded @font-face rules
for font in .pagemd/fonts/*.woff2; do
  name=$(basename "$font" .woff2)
  echo "@font-face {"
  echo "  font-family: 'Inter';"
  echo "  font-weight: 400;  /* Adjust per file: 400=Regular, 700=Bold */"
  echo "  src: url('data:font/woff2;base64,$(base64 -w0 "$font")') format('woff2');"
  echo "}"
done > .pagemd/styles/fonts.css
```

Or manually create the file:

```css
/* .pagemd/styles/fonts.css */
@font-face {
  font-family: 'Inter';
  font-style: normal;
  font-weight: 400;
...
> **Note:** Base64 encoding increases file size by ~33%. A 100KB font becomes ~133KB in CSS. Include only the weights you actually use.

#### Step 3: Include Fonts in Profile

Reference the font CSS file **before** other CSS files (order matters):
...
```json
{
  ...
  "resources": {
    "css": [
      "${workspaceFolder}/.pagemd/styles/fonts.css",
      "${workspaceFolder}/.pagemd/styles/my-styles.css"
    ]
  }
}
```
...
In your style CSS, reference the font family:

```css
/* .pagemd/styles/my-styles.css */
body {
  font-family: 'Inter', -apple-system, 'Segoe UI', sans-serif;
}
```

The embedded font will be used; fallbacks only apply if base64 decoding fails.

#### Verify Fonts in PDF

After building a PDF, check which fonts are embedded:

```bash
# On Linux/WSL with poppler-utils installed
pdffonts output.pdf

# Alternative: search PDF content
strings output.pdf | grep FontName
```

Expected output shows your embedded font:
```
/FontName /AAAAAA+Inter-Regular
/FontName /BAAAAA+Inter-Bold
```

#### Example: Book Profile with Inter Font

The built-in `book-wiki` profile embeds Inter font. Structure:

```
project/pagemd/
  fonts/inter/               # Font source files
    Inter-Regular.woff2
    Inter-Bold.woff2
    LICENSE.txt
  styles/
    book-fonts.css           # Base64-encoded @font-face rules
    book.css                 # Main styles (uses Inter)
  profiles/
    book-wiki.json           # References book-fonts.css, then book.css
```
...
```json
{
  "resources": {
    "css": [
      "${cliPath}/styles/book-fonts.css",
      "${cliPath}/styles/book.css"
    ]
  }
}
```
